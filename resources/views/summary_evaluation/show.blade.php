...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">ผลสรุปผลการประเมิน {{ $summary_evaluation->id }}</div>
                    <div class="card-body">

                        <a href="{{ url('/summary_evaluation') }}" title="Back"><button class="btn btn-warning btn-sm"><i class="fa fa-arrow-left" aria-hidden="true"></i> กลับ</button></a>
                        <a href="{{ url('/summary_evaluation/' . $summary_evaluation->id . '/edit') }}" title="Edit Summary_evaluation"><button class="btn btn-primary btn-sm"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> แก้ไข</button></a>

                        <form method="POST" action="{{ url('summary_evaluation' . '/' . $summary_evaluation->id) }}" accept-charset="UTF-8" style="display:inline">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
                            <button type="submit" class="btn btn-danger btn-sm d-none" title="Delete Summary_evaluation" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</button>
                        </form>
                        <br/>
                        <br/>
