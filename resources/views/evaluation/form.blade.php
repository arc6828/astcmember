...
<div class="form-group {{ $errors->has('article_id') ? 'has-error' : ''}}">
    ...
    <input class="form-control" name="article_id" type="text" id="article_id" value="{{ isset($evaluation->article_id) ? $evaluation->article_id : request('article_id') }}" readonly >
    {!! $errors->first('article_id', '<p class="help-block">:message</p>') !!}
</div>

<div class="form-group {{ $errors->has('user_id') ? 'has-error' : ''}}">
    <label for="user_id" class="control-label">{{ 'ผู้เขียนบทความ' }}</label>
    <input class="form-control d-none" name="user_id" type="text" id="user_id" value="{{ isset($evaluation->user_id) ? $evaluation->user_id : Auth::id()}}" readonly >
    <input class="form-control" value="{{ isset($evaluation->user_id) ? $evaluation->user->name : Auth::user()->name }}" readonly>
