...
<div class="form-group {{ $errors->has('name_aj') ? 'has-error' : ''}}">
    <label for="name_aj" class="control-label">{{ 'รายชื่อทุกคนในบทความ' }}</label>
...
<div class="form-group {{ $errors->has('tel_aj') ? 'has-error' : ''}}">
    <label for="tel_aj" class="control-label">{{ 'เบอร์โทรศัพท์ผู้รับผิดชอบหลัก' }}</label>
    <input class="form-control" name="tel_aj" type="text" id="tel_aj" value="{{ isset($article->tel_aj) ? $article->tel_aj : ''}}" >
...
<div class="form-group">
    <input class="btn btn-primary" type="submit" name="status"  value="{{ $formMode === 'edit' ? 'Update' : 'Create' }}">
</div>
